...
async function scrapeOrders() {
  ...
  try {
    ...
    for (let i = 0; i < orderLinks.length; i++) {
      ...
      // Extract all items — each item is a grid div inside the purchased items section
      const items = await page.$$eval(
        'div.grid.gird-cols-1',
      ...
      if (items.length === 0) {
        // Fallback: single row with no item detail
        results.push([orderId, date, '', '', total]);
      } else {
        // One row per item, repeating order-level fields
